...
@section('title', __('newOrders.title').'(可轉單)')

@section('content_header')
    <h1 class="m-0 text-dark">{{ __('newOrders.header').'(可轉單)' }}</h1>
@stop
...
<form id="eapply-form" action="{{ route('newOrders.exports') }}" method="POST" onSubmit="checkform()">
    @method('PUT')
    @csrf
    <table id="eapplyTable" class="stripe row-border order-column nowrap" style="width:100%">
        <thead>
            <tr>
                ...
                <th>{{ __('newOrders.id') }}</th>
                <th>{{ __('newOrders.reseller') }}</th>
                <th>{{ __('newOrders.name') }}</th>
                <th>{{ __('newOrders.phone') }}</th>
                <th>{{ __('newOrders.project') }}</th>
                <th>{{ __('newOrders.payment') }}</th>
                <th>{{ __('newOrders.created_at') }}</th>
            </tr>
        </thead>
        <tbody>
@php
...
@endphp
          @foreach($newOrders as $eapply)
            <tr>
                ...
                <td>{{ ($eapply->payment == 1) ? __('newOrders.payment_tt') : __('newOrders.payment_credit') }}</td>
