...
<form id="massOrder-form" action="{{ route('neworders.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
        @livewire('new-order')
        <div class="raw card-group">
           <x-adminlte-input name="name" label="{{ __('newOrders.name') }}" fgroup-class="col-md-6" />
        ...
        <div class="raw card-group">
           ...
           <x-adminlte-input name="unified_number" label="{{ __('newOrders.unified_number') }}" fgroup-class="col-md-6" />
        </div>
        <div class="raw card-group">
        ...
        <div class="raw card-group">
           <x-adminlte-textarea name="memo" label="{{ __('newOrders.memo') }}" fgroup-class="col-md-12" />
        </div>
        <div class="raw card-group">
          <p><strong>{{ __('newOrders.orderitems') }}</strong></p>
          <table class="table table-bordered" id="productsTable">
            <tr>
                 <td>{{ __('newOrders.product') }}</td>
                 <td>{{ __('newOrders.price') }}</td>
                 <td>{{ __('newOrders.count') }}</td>
                 <td>{{ __('newOrders.amount') }}</td>
                 <td>{{ __('newOrders.action') }}</td>
            </tr>
                <tr>
                    <td><input type="text" name="products[0]['product']"  id="product[0]" class="form-control" />
                        <x-adminlte-button label="{{ __('newOrders.product') }}" data-toggle="modal" data-target="#productsModal"
                          class="bg-primary" data-whatever="0" />
                    </td>
                    <td><input type="number" name="products[0]['price']" class="form-control"  /></td>
                    <td><input type="number" name="products[0]['count']" class="form-control"  /></td>
...
<script type="text/javascript">
    ...
    $("#productAdd").click(function () {
        ...
        var str = '<tr><td><input type="text" name="products[' + i + '][\'product\']" id="product[' + i + ']" class="form-control" />';
            ...
            str += '<td><input type="number" name="products[' + i + '][\'price\']" class="form-control" /></td>';
            str += '<td><input type="number" name="products[' + i + '][\'count\']" class="form-control" /></td>';
