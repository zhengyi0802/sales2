...
@section('title', __('lockinstallers.title').'(可轉單)')

@section('content_header')
    <h1 class="m-0 text-dark">{{ __('lockinstallers.header').'(可轉單)' }}</h1>
@stop
...
<form id="eapply-form" action="{{ route('lockinstallers.exports') }}" method="POST" onSubmit="checkform()">
    @method('PUT')
    @csrf
    <table id="eapplyTable" class="stripe row-border order-column nowrap" style="width:100%">
        <thead>
            <tr>
                ...
                <th>{{ __('lockinstallers.id') }}</th>
                <th>{{ __('lockinstallers.reseller') }}</th>
                <th>{{ __('lockinstallers.name') }}</th>
                <th>{{ __('lockinstallers.phone') }}</th>
                <th>{{ __('lockinstallers.project') }}</th>
                <th>{{ __('lockinstallers.payment') }}</th>
                <th>{{ __('lockinstallers.created_at') }}</th>
            </tr>
        </thead>
        <tbody>
@php
...
@endphp
          @foreach($lockinstallers as $lockInstaller)
            <tr>
                <td><input type="checkbox" id="ids" name="ids[{{ $i }}]" value="{{ $lockInstaller->id }}" ></td>
                <td>{{ $lockInstaller->id }}</td>
                <td>{{ mb_substr($lockInstaller->reseller->name, 0, 4) }}</td>
                <td>{{ $lockInstaller->name }}</td>
                <td>{{ $lockInstaller->phone }}</td>
                <td>{{ $lockInstaller->project->name }}</td>
                <td>{{ ($lockInstaller->payment == 1) ? __('lockinstallers.payment_tt') : __('lockinstallers.payment_credit') }}</td>
