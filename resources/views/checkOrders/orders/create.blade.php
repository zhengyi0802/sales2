            ...
            <tr class="form-group col-md-4">
                ...
                <td><select id="project_id" name="project_id">
                      @foreach ($projects as $project)
                         <option value="{{ $project->id }}" {{ ($customer->order->project_id == $project->id) ? "selected" : null }} >{{ $project->name }}</option>
                      @endforeach
                </select></td>
            </tr>
            <tr class="form-group col-md-4">
                ...
                <td><select id="product_id" name="product_id" >
                      @foreach ($productModels as $model)
                         <option value="{{ $model->id }}" {{ ($customer->order->product_id == $model->id) ? "selected" : null }} >{{ $model->name }}</option>
                      @endforeach
                </select></td>
            </tr>
            <tr class="form-group col-md-4">
                ...
                <td><select id="extra_id" name="extra_id[]" multiple="multiple" size="10">
                      ...
                      @foreach ($extras as $extra)
                         <option value="{{ $extra->id }}" {{ in_array($extra->id, $extras2) ? "selected" : null }} >{{ $extra->name }}</option>
                      @endforeach
                </select></td>
            </tr>
          </table>
