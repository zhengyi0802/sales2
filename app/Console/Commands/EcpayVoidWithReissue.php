...
class EcpayVoidWithReissue extends Command
{
    private $env = 'test';
    /**
     ...
     * @var string
     */
    protected $signature = 'ecpay:voidWithReissue';
    ...
    {
        $this->VoidWithReissue();
    }

    public function VoidWithReissue()
    {
        $Invoice = new ECPayInvoice('test');

        $apply = EApply::find(599);
        $issueModel = $Invoice->buildIssueData($apply);
        $voidModel = [
