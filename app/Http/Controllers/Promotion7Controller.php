...
class Promotion7Controller extends Controller
{
    //
    public function index(Request $request)
    {
        ...
        if ($user->role == UserRole::Reseller || $user->role == UserRole::Sales) {
            if ($flow && ($flow == 8 || $flow == 9)) {
                $promotions = HpPromotion::where('proj_id', 7)->where('reseller_id', $user->sales->id)->where('flow', 8)->orWhere('flow', 9)->where('status', true)->get();
                return view('promotion7.index2', compact('promotions'));
            } else {
                $promotions = HpPromotion::where('proj_id', 7)->where('reseller_id', $user->sales->id)->where('status', true)->get();
                return view('promotion7.index', compact('promotions'));
            }
        } else {
            if ($flow && $flow == 14) {
                $promotions = HpPromotion::where('proj_id', 7)->where('flow', 14)->where('status', true)->get();
                return view('promotion7.index', compact('promotions'));
            } else if ($flow && ($flow == 8 || $flow == 9)) {
                $promotions = HpPromotion::where('proj_id', 7)->where('flow', 8)->where('status', true)->orWhere('flow', 9)->get();
                return view('promotion7.index2', compact('promotions'));
            } else {
                if ( $user->role <= UserRole::Manager ) {
                    $promotions = HpPromotion::where('proj_id', 7)->get();
                } else {
                    $promotions = HpPromotion::where('proj_id', 7)->where('status', true)->get();
                }
                return view('promotion7.index', compact('promotions'));
            }
        }
    ...
    public function show(HpPromotion $promotion7)
    {
        return view('promotion7.show', compact('promotion7'));
    }

    public function edit(HpPromotion $promotion7)
    {
        try {
              $products = HpProduct::where('status', true)->get();
              $results = EcpayResult::where('trade_no', $promotion7->trade_no)->first();
              $gifts = array();
              if ($promotion7->gifts != null) {
                  $gifts = json_decode($promotion7->gifts);
              }
              $bundles = array();
              if ($promotion7->bundles != null) {
                  $bundles = json_decode($promotion7->bundles);
              }
        ...
        return view('promotion7.edit', compact('promotion7'))
    ...
    public function update(Request $request, HpPromotion $promotion7)
    {
        $data = $request->all();
        try {
              if (isset($data['paid'])) {
                  $data['remain'] = $promotion7->total-$data['paid'];
              } else {
                  $data['remain'] = 0;
              }
              if (($data['remain'] == 0) && ($promotion7->paid > 0)) {
                 if (isset($data['product_id']) && $data['product_id'] != $promotion7->product_id) {
                     $product = HpProduct::find($data['product_id']);
                     $price = $product->price;
                     //$total = $price * $promotion->amount;
                     $data['remain'] = $promotion7->total-$promotion7->paid;
                     $promotion7->update($data);
                     return redirect()->route('promotion7.edit', compact('promotion7'));
                 }
             }
             $promotion7->update($data);
        } catch (QueryException $e) {
    ...
    public function exports(Request $request)
    {
        ...
        if ($ids == null) {
            return redirect()->route('promotion7.index');
        }
        foreach($ids as $id) {
            $order_id = '97'.sprintf('%06d', $id);
            ...
            if ($response) {
                ...
                try {
                    ...
                    $export_data = [
                          ...
                          'prom_id'      => $id,
                          'proj_id'      => 7,
                          'path'         => 'exports@promotion7Controller',
                          'ecount'       => 1,
                    ...
                    return redirect()->route('promotion7.index')->with('error', $error);
                }
            }
        }
        ...
        if ($response) {
            ...
            try {
                ...
                $export_data = [
                          ...
                          'prom_id'      => $id,
                          'proj_id'      => 7,
                          'path'         => 'export@promotion7Controller',
                          'ecount'       => 1,
                          'created_by'   => auth()->user()->id,
                ];
                GasExport::create($export_data);
    ...
    public function destroy(HpPromotion $promotion7)
    {
        try {
              if ($promotion7->status == false) {
                  if ($promotion7->EcpayInfo != null) {
                      $promotion7->delete();
                  }
    ...
    {
        $id = '97'.sprintf('%06d', $promotion->id);
        $arrs = array(
