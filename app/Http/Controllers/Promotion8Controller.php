...
class Promotion8Controller extends Controller
{
    //
    public function index(Request $request)
    {
        ...
        if ($user->role == UserRole::Reseller || $user->role == UserRole::Sales) {
            if ($flow && ($flow == 8 || $flow == 9)) {
                $promotions = HpPromotion::where('proj_id', 8)->where('reseller_id', $user->sales->id)->where('flow', 8)->orWhere('flow', 9)->where('status', true)->get();
                return view('promotion8.index2', compact('promotions'));
            } else {
                $promotions = HpPromotion::where('proj_id', 8)->where('reseller_id', $user->sales->id)->where('status', true)->get();
                return view('promotion8.index', compact('promotions'));
            }
        } else {
            if ($flow && $flow == 14) {
                $promotions = HpPromotion::where('proj_id', 8)->where('flow', 14)->where('status', true)->get();
                return view('promotion8.index', compact('promotions'));
            } else if ($flow && ($flow == 8 || $flow == 9)) {
                $promotions = HpPromotion::where('proj_id', 8)->where('flow', 8)->where('status', true)->orWhere('flow', 9)->get();
                return view('promotion8.index2', compact('promotions'));
            } else {
                if ( $user->role <= UserRole::Manager ) {
                    $promotions = HpPromotion::where('proj_id', 8)->get();
                } else {
                    $promotions = HpPromotion::where('proj_id', 8)->where('status', true)->get();
                }
                return view('promotion8.index', compact('promotions'));
            }
        }
    ...
    public function show(HpPromotion $promotion8)
    {
        return view('promotion8.show', compact('promotion8'));
    }

    public function edit(HpPromotion $promotion8)
    {
        try {
              $products = HpProduct::where('status', true)->get();
              $results = EcpayResult::where('trade_no', $promotion8->trade_no)->first();
              $gifts = array();
              if ($promotion8->gifts != null) {
                  $gifts = json_decode($promotion8->gifts);
              }
              $bundles = array();
              if ($promotion8->bundles != null) {
                  $bundles = json_decode($promotion8->bundles);
              }
        ...
        return view('promotion8.edit', compact('promotion8'))
    ...
    public function update(Request $request, HpPromotion $promotion8)
    {
        $data = $request->all();
        try {
              if (isset($data['paid'])) {
                  $data['remain'] = $promotion8->total-$data['paid'];
              } else {
                  $data['remain'] = 0;
              }
              if (($data['remain'] == 0) && ($promotion8->paid > 0)) {
                 if (isset($data['product_id']) && $data['product_id'] != $promotion8->product_id) {
                     $product = HpProduct::find($data['product_id']);
                     $price = $product->price;
                     //$total = $price * $promotion->amount;
                     $data['remain'] = $promotion8->total-$promotion8->paid;
                     $promotion8->update($data);
                     return redirect()->route('promotion8.edit', compact('promotion8'));
                 }
             }
             $promotion8->update($data);
        } catch (QueryException $e) {
    ...
    public function exports(Request $request)
    {
        ...
        if ($ids == null) {
            return redirect()->route('promotion8.index');
        }
        foreach($ids as $id) {
            $order_id = '98'.sprintf('%06d', $id);
            ...
            if ($response) {
                ...
                try {
                    ...
                    $export_data = [
                          ...
                          'prom_id'      => $id,
                          'proj_id'      => 8,
                          'path'         => 'exports@promotion8Controller',
                          'ecount'       => 1,
                    ...
                    return redirect()->route('promotion8.index')->with('error', $error);
                }
            }
        }
        ...
        if ($response) {
            ...
            try {
                ...
                $export_data = [
                          ...
                          'prom_id'      => $id,
                          'proj_id'      => 8,
                          'path'         => 'export@promotion8Controller',
                          'ecount'       => 1,
                          'created_by'   => auth()->user()->id,
                ];
                GasExport::create($export_data);
    ...
    public function destroy(HpPromotion $promotion8)
    {
        try {
              if ($promotion8->status == false) {
                  if ($promotion8->EcpayInfo != null) {
                      $promotion8->delete();
                  }
    ...
    {
        $id = '98'.sprintf('%06d', $promotion->id);
        $arrs = array(
